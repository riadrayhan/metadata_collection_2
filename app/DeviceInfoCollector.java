 ...
 * - OS version, API level, security patch
 * - Device age estimate (first boot time)
 * - Root/jailbreak detection
 * - SIM swap count (from sim_history table)
 * - Factory reset detection
 * - Screen info, RAM, storage
 * - Battery health
 */
public class DeviceInfoCollector {
    ...
    public void collect() {
        ...
        long deviceUptime = SystemClock.elapsedRealtime(); // ms since boot
        String uptimeDays = String.valueOf(deviceUptime / (1000 * 60 * 60 * 24));

        // Root detection
        boolean isRooted = checkRoot();

        // SIM swap count from DB
        ...
        db.insertDeviceInfo(
            deviceId, brand, model, manufacturer, device, hardware,
            osVersion, String.valueOf(apiLevel), securityPatch, buildFingerprint,
            String.valueOf(firstInstallTime), uptimeDays,
            isRooted ? "YES" : "NO",
            String.valueOf(simSwapCount),
            String.valueOf(factoryResetIndicator),
            screenInfo, ramInfo, storageInfo, batteryInfo, networkType,
            timezone, language, country, timestamp
        );
    }
    ...
    private long getFirstInstallTime() {
        ...
        if (savedTime == 0) {
            // Use app first install time as proxy
            try {
    ...
    private int detectFactoryReset() {
        // Track boot count as factory reset proxy
        SharedPreferences prefs = context.getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE);
        int bootCount = prefs.getInt(KEY_BOOT_COUNT, 0);

        // If app data was cleared (boot_count reset to 0), possible factory reset
        if (bootCount == 0) {
            // Check if this is truly first install
            try {
                long firstInstall = context.getPackageManager()
                    .getPackageInfo(context.getPackageName(), 0).firstInstallTime;
                long now = System.currentTimeMillis();
                // If app was just installed (within last hour), not a reset
                if ((now - firstInstall) < 3600000) {
                    bootCount = 0; // New install
                } else {
                    bootCount = -1; // Possible reset - data was cleared
                }
            } catch (Exception e) {
                bootCount = 0;
            }
